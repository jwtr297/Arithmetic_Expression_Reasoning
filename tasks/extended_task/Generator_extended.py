...
from arithmetic_experiments.extended_task_experiment.utils import Config
from modular_data_generation_extended import generate_noisy_twosteps_dataset_modp, generate_noisy_twosteps_dataset_modp_1, generate_noisy_twosteps_varied_dataset_modp, generate_noisy_twosteps_dataset_modp_extended
from arithmetic_experiments.extended_task_experiment.Multiplication_addition_training_extended import decode_expressions
...
def _write_chunk_to_memmap(args):
    ...
    data, _, _ = generate_noisy_twosteps_dataset_modp_extended(N=chunk_size, p=modulus, p1=p1, p2=p2, extended_expression = True)
    #data, _, _ = generate_noisy_twosteps_varied_dataset_modp(N=chunk_size, p=modulus, p1=p1, p2=p2)
    data = data.to(torch.uint8)                  # CPU uint8
    arr = data.numpy()                          
...
def build_and_save_memmap(
    ...
    first_size = sizes[0]
    data0, _, _ = generate_noisy_twosteps_dataset_modp_extended(N=first_size, p=modulus, p1=p1, p2=p2, extended_expression = True)
