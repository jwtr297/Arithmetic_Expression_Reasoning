...
from arithmetic_experiments.minimal_task_experiment.utils import matching_scores, TrainingInfo, autoregressive_extend, induction_score, simplification_head_score, eval_opposite_structure
from tasks.minimal_task.modular_data_generation_wop import generate_noisy_twosteps_dataset_modp, generate_noisy_twosteps_dataset_modp_wop, ModularVocabulary,compute_e1_e2_distributions, compute_short_E3_distributions, generate_noisy_twosteps_varied_dataset_modp
from pathlib import Path
from arithmetic_experiments.minimal_task_experiment.unfaithfulness_metrics_wop import measure_e2_all_digits_intervention_ce_modp, measure_e2_all_digits_intervention_ce_modp_short_wop, measure_e1_ab_digits_intervention_ce_modp_short_wop, compute_e3_entropy_sharpness_modp_short,measure_e3_agreement_with_e1_e2_modp, autoreg_generate_from_firstded_to_stop_1, autoreg_generate_from_secondded_to_stop_1, proportion_E1_eq_r_but_E2_neq_r_1,last_layer_attn_ffn_no_residual, shuffle_e2_first_digit_modp_short, get_last_layer_QK, select_E1_a_in_firstN, proportion_E2_eq_r_1, proportion_E1_eq_r_1, select_E1_firstdigit_and_E1_not_eq_E2, select_E1_not_eq_E2, measure_e2_addup_intervention_ce_modp_short, proportion_E1_eq_r_but_E2_neq_r_1, proportion_E1_neq_r_but_E2_eq_r_1, proportion_E1_eq_r_and_E2_eq_r_1, proportion_E1_neq_r_and_E2_neq_r_1, compute_structure_accuracy_from_first_deduct_seq, proportion_E2_E3_joint_cases_wop
...
def build_token_display_map_wop(
    modulus: int,
    vocab: ModularVocabulary | dict | None = None,
) -> dict:
    
    if isinstance(vocab, ModularVocabulary):
    ...
    if vocab_tokens is not None:
        numeric_tokens = [int(k) for k in vocab_tokens.keys() if k.isdigit()]
        if numeric_tokens:
            modulus = max(numeric_tokens) + 1

    # Special tokens order (aligned with new vocabulary: no LP/RP)
    special_order = {
        "PLUS": 0,
        "MINUS": 1,
        "MUL": 2,
    ...
    # How to render each special token as a string
    render_map = {
        "PLUS": "+",
        "MINUS": "-",
        "MUL": "*",
    ...
    for name, offset in special_order.items():
        if vocab_tokens is not None and name in vocab_tokens:
            token_map[vocab_tokens[name]] = render_map[name]
        else:
            token_map[modulus + offset] = render_map[name]
...
def save_checkpoints(
    ...
    ckpt_path = os.path.join(OUT_DIR, "MA_checkpoints")
...
def train_with_dataloader(
    ...
    train_loader, N=1000, p1=0.05, p2=0.05, training_info=None, modulus: int = 11, vocab: ModularVocabulary | dict | None = None  
):

    test_set, test_vocab, test_set_info = generate_noisy_twosteps_dataset_modp_wop(
        N, modulus, p1, p2, device=config.device
    )  

    test_neg, test_neg_vocab, test_pos_info = generate_noisy_twosteps_dataset_modp_wop(  
        N, modulus, 0, 0, device=config.device
    )
    
    test_pos = shuffle_e2_first_digit_modp_short(test_neg,p=modulus)

  
    if vocab is None:  ## CHANGED
        vocab = test_vocab  ## CHANGED
    token_map = build_token_display_map_wop(modulus, vocab)  ## CHANGED
    ...
    for epoch, (src_cpu, mask_cpu) in enumerate(
        ...
        evaluation_ready = should_evaluate(epoch)
        #evaluation_ready = (epoch%1000==0)
        if evaluation_ready:  
            ...
            with torch.no_grad():
                ...
                avg_KL, _, avg_inv,_ = measure_e2_all_digits_intervention_ce_modp_short_wop(model, test_set, modulus)
                
                
                avg_KL_1, _, avg_inv_1,_ = measure_e1_ab_digits_intervention_ce_modp_short_wop(model, test_set, modulus)
                ...
                #entropy
                _, sharpness = compute_e3_entropy_sharpness_modp_short(model, test_set, modulus) 
                _, sharpness_pred = compute_e3_entropy_sharpness_modp_short(model, pred_autoreg, modulus) 
    
      
                ratio_E2_eq_E3_eq, ratio_E2_eq_E3_neq, ratio_E2_neq_E3_eq, ratio_E2_neq_E3_neq = proportion_E2_E3_joint_cases_wop(pred_autoreg, test_set,modulus)

                hidden_state_pos = last_layer_attn_ffn_no_residual(model, test_pos.long()[0:1000], use_training_mode=False)

                hidden_state_neg = last_layer_attn_ffn_no_residual(model, test_neg.long()[0:1000], use_training_mode=False)

                #1. indicator 
                indicator_metric = (hidden_state_pos[:,-3,:].abs().amax(dim=1)-hidden_state_neg[:,-3,:].abs().amax(dim=1)).mean()
