...
@torch.no_grad()
def measure_e1_bc_digits_intervention_ce_modp_short(
    model,
    data: torch.Tensor,                       # [N, L] int64 tokens
    p: int,                                   # modulus p (digits are 0..p-1)
    num_variants: int = 50,                   # number of variants per sample
    ...
    """
    Intervention metric for short-form a * (b ± c) --> a * d --> r:
      - Only intervene on 'b' token in E1 (不动 a 和 c).
    ...
    # Helper: extract E3 logits (predict r at index -2, so read logits at -3)
    def _e3_logits_from(model_outputs_logits: torch.Tensor) -> torch.Tensor:
        return model_outputs_logits[:, -3, :][:, :p]  # [B, p]
    ...
    # === 2) Build intervention variants (only b in E1) ===
    modified_seqs = []
    modified_src_idx = []
    ...
    for i in range(N):
        ...
        # Find start and first DED (E1 结束位置)
        nonpad_idx = (seq != PAD_ID).nonzero(as_tuple=False)
        if nonpad_idx.numel() == 0:
            continue
        start_idx = int(nonpad_idx[0].item())
        ...
        # 期望 E1 中有 a, b, c 三个 digit
        if rel_digits.numel() < 2:
            # 连 b 都没有就跳过
            continue

        # b 是 E1 中的第 2 个 digit（索引 1）
        # 用 [1:2] 保持一个长度为 1 的张量，后面循环逻辑不用改
        rel_b = rel_digits[1:2]
        b_abs_pos = rel_b + start_idx
...
def shuffle_e2_first_digit_modp_short(
    ...
    # Special token IDs (NO parenthesis defined now)
    PLUS_ID  = p
    MINUS_ID = p + 1
    MUL_ID   = p + 2
    DED_ID   = p + 3
    ...
    for i in range(N):
        ...
        # 2) Locate the two DED markers delimiting E2
        idx_ded_all = (seq == DED_ID).nonzero(as_tuple=False).squeeze(-1)
        if idx_ded_all.numel() < 2:
            continue
        ...
        # 4) Build candidates excluding original when possible
        if orig_product in allowed_digits and len(allowed_digits) > 1:
            candidates = [d for d in allowed_digits if d != orig_product]
        else:
            candidates = allowed_digits
...
def select_E1_firstdigit_and_E1_not_eq_E2(
    seqs: torch.Tensor,
    a_values: Sequence[int],
    p: int = 11,
    max_n: int = 1000,  
    ...
    for i in range(N):
        ...
        if len(digits_in_e1) < 1:
            continue
        first_digit = digits_in_e1[0]

        if first_digit not in allowed_a:
            continue
...
@torch.no_grad()
def measure_e2_all_digits_intervention_ce_modp_short_wop(
    model,
    data: torch.Tensor,                       
    p: int,                                   # modulus p (digits are 0..p-1)
    num_variants: int = 50,                   # number of random replacements per sample
    ...
    """
    Evaluate robustness for SHORT layout by intervening ONLY the E2 product token,
    for the parenthesis-free sequence format.

    Assumed short-layout format (no parentheses), e.g.:
        a, MUL, b, op, c,
        DEDUCT, product, op, c,
        DEDUCT, result, END

    - E2 is the span between the first and second DEDUCT tokens.
    - Inside E2, the product is identified as the FIRST digit token (0..p-1).
    - E3 (final result) is predicted at the position just *after* the second DEDUCT,
      so we use the logits at the second DEDUCT position (time step t predicting t+1).
    """
    ...
    # Special token IDs for mod p with the new vocabulary (no parentheses):
    # 0..p-1: digits
    PLUS_ID  = p
    MINUS_ID = p + 1
    MUL_ID   = p + 2
    ...
    # --- Short layout: result is t+1 after the second DED,
    #     so we take logits at the second DED position (index -3) ---
    def _e3_logits_from(model_outputs_logits: torch.Tensor) -> torch.Tensor:
        # model_outputs_logits: [B, L, V]
        # We use time step -3 (the second DED) to predict the next token (the result)
        # and restrict to digit classes [0..p-1].
        return model_outputs_logits[:, -3, :][:, :p]  # [B, p]
...
def proportion_E2_E3_joint_cases_wop(
    seqs_true: torch.Tensor,
    seqs_pred: torch.Tensor,
    p: int = 11,
    e2_start_idx: int = 6,   # for short, no-parentheses layout: E2 starts at product
    e2_end_idx: int = 9,     # slice [6:9] -> product, op, c
    r_idx: int = 10,         # result index
) -> Tuple[float, float, float, float]:
    """
    Compute the joint proportions of four cases:
        (E2_eq, r_eq), (E2_eq, r_neq), (E2_neq, r_eq), (E2_neq, r_neq),

    where:
      - E2_eq: E2 segment (between e2_start_idx and e2_end_idx) matches exactly
               between seqs_true and seqs_pred.
      - r_eq:  final result token at r_idx matches between seqs_true and seqs_pred.

    Assumed short-layout without parentheses, e.g.:
        a, MUL, b, op, c,
        DED, product, op, c,
        DED, result, END
    ...
    # New vocabulary: PAD_ID = p + 5 (no LP/RP anymore)
    PAD_ID = p + 5
    ...
    for i in range(N):
        ...
        # If result is PAD in either, skip
        if r_true == PAD_ID or r_pred == PAD_ID:
            continue
...
@torch.no_grad()
def measure_e1_ab_digits_intervention_ce_modp_short_wop(
    model,
    data: torch.Tensor,                       # [N, L] int64 tokens
    p: int,                                   # modulus p (digits are 0..p-1)
    num_variants: int = 50,                   # number of variants per sample
    ...
    """
    Intervention metric for short-form (a * b) ± c expressions (parenthesis-free):
      - Only intervene on 'a' and 'b' tokens in E1.

    Token IDs under mod p (no parentheses):
      digits: 0 .. p-1
      PLUS:   p
      MINUS:  p + 1
      MUL:    p + 2
    ...
    # Special token IDs in the new vocabulary
    DED_ID = p + 3
    PAD_ID = p + 5
    ...
    # Helper: extract E3 logits (result position is predicted from the second DED at -3)
    def _e3_logits_from(model_outputs_logits: torch.Tensor) -> torch.Tensor:
        # Restrict to digit classes [0..p-1]
        return model_outputs_logits[:, -3, :][:, :p]  # [B, p]
    ...
    for i in range(N):
        ...
        # Find the first non-PAD position as sequence start
        nonpad_idx = (seq != PAD_ID).nonzero(as_tuple=False)
        if nonpad_idx.numel() == 0:
            continue
        start_idx = int(nonpad_idx[0].item())

        # Locate the first DED marker to delimit E1
        ded_idx_all = (seq == DED_ID).nonzero(as_tuple=False).squeeze(-1)
        if ded_idx_all.numel() == 0:
            continue
        first_ded_candidates = ded_idx_all[ded_idx_all >= start_idx]
        ...
        # E1 slice: from start_idx up to (but not including) the first DED
        e1_slice = seq[start_idx:first_ded]

        # Digits in E1 (we assume the first two digit tokens correspond to a and b)
        rel_digits = (e1_slice < p).nonzero(as_tuple=False).squeeze(-1)
        if rel_digits.numel() < 2:
            continue
        ...
        # Create num_variants by randomly changing a and b
        for _ in range(num_variants):
            new_seq = seq.clone()
            skip_variant = False
...
@torch.no_grad()
def last_layer_attn_ffn_no_residual(
    model,
    src: torch.Tensor,
    *,
    use_training_mode: bool = False,
