...
def generate_noisy_twosteps_dataset_modp_extended(
    N: int,
    p: int,
    p1: float,
    p2: float,
    *,
    ...
    """
    Generate two-step modular arithmetic expressions with noise.

    Layout (extended_expression = True):
        (a +/- b) * c +/- d   ->   s +/- d   ->   r#
      tokens:
        0: LP
        1: a
    ...
    Layout (extended_expression = False, short form):
        (a * b) +/- c   ->   s +/- c   ->   r#
      tokens:
        0: LP
        1: a
    ...
    Noise:
      - E1 noise (p1): uniformly on {a, b, c}, i.e. token indices {1,3,6}.
      - E2 noise (p2): only on s (the intermediate result), i.e. index 10 (extended)
                       or index 8 (short).
    """
    if N <= 0:
        raise ValueError("N must be a positive integer.")
    ...
    if extended_expression:
        base_len = 16
        solution_start_index = 10  # index of s in the first DEDUCT segment
        s_idx_in_e2 = 10           # s position inside E2 for extended layout
    else:
        base_len = 14
        solution_start_index = 8   # index of s in the first DEDUCT segment
        s_idx_in_e2 = 8            # s position inside E2 for short layout

    # E1 noise indices: a, b, c (for both layouts)
    e1_noise_candidates = (1, 3, 6)
    ...
    for i in range(N):
        if extended_expression:
            ...
            first_partial = (a + b) % p if first_op == PLUS_ID else (a - b) % p
            s = (first_partial * c) % p
            r = (s + d) % p if second_op == PLUS_ID else (s - d) % p

            # Build token sequence: (a +/- b)*c +/- d -> s +/- d -> r#
            tokens: List[int] = [
                LP_ID, a, first_op, b, RP_ID, MUL_ID, c, second_op, d,
                DEDUCT_ID,
                s,
            ...
            s = (a * b) % p
            r = (s + c) % p if op == PLUS_ID else (s - c) % p
        ...
        # ---- E1 noise: uniformly on {a, b, c} ----
        if rng.random() < p1:
            idx = rng.choice(e1_noise_candidates)
            tokens[idx] = _sample_different(rng, 0, p - 1, exclude=tokens[idx])

        # ---- E2 noise: ONLY corrupt s ----
        if rng.random() < p2:
            tokens[s_idx_in_e2] = _sample_different(
                rng, 0, p - 1, exclude=tokens[s_idx_in_e2]
            )
